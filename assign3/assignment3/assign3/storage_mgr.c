...
RC writeBlock_n (int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage)
{
	...
	if(fHandle != NULL)
	{
		if(fp != NULL)
		{
			fseek(fp,(pageNum)*PAGE_SIZE,SEEK_SET+off);
			for(i = 0;i<4096;i++)
			{
				fputc(memPage[i],fp);
				printf("%c",memPage[i]);
