...
printf("inside serialize schema;");
...
serializeAttr(Record *record, Schema *schema, int attrNum)
{
 ...
 // printf("offset value:: %d\n",offset);
  ...
  switch(schema->dataTypes[attrNum])
	...
	//sprintf(d,"%s:%i", schema->attrNames[attrNum], val);
	//printf("d is ********* ::%s",d);
      }
      break;
    case DT_STRING:
	...
	//printf("result in serializeAttr:: %s\n\n",result);
	free(buf);
      }
      break;
    case DT_FLOAT:
...
attrOffset (Schema *schema, int attrNum, int *result)
{
  int offset = 0;
  int attrPos = 0;
  //printf("inside attroffset method;");
...
char * 
serializeRecord_n(Record *record, Schema *schema)
{
  VarString *result;
	char r[12];
  MAKE_VARSTRING(result);
  ...
  APPEND(result, "[%d-%d] :", record->id.page, record->id.slot);
  //APPEND(result,"%s",record->data); 
  /*for(i = 0; i < 12; i++)
   {
      APPEND(result,"%c",record->data[i]);
      //APPEND(result, "%s", (i == 0) ? "" : ",");
  }*/
	
    memcpy(r,record->data,12);
	    /*for(i = 0; i < schema->numAttr; i++)
    {
      //APPEND_STRING(result, record->data);
      APPEND(result, "%s", (i == 0) ? "" : ",");
    }
  */
  
  APPEND_STRING(result, r);

	/*FILE * fp = fopen("xxx.txt","wb");
      fwrite(result,1,25,fp);	
	fclose(fp);*/
