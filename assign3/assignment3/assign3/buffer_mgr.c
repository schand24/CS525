...
RC pinPage (BM_BufferPool *const bm, BM_PageHandle *const page,const PageNumber pageNum)
{
	...
	//printf("....................INSIDE PIN PAGE FUNCTION......%d\n",pageNum);
	k = bm->numPages;
	for(i=0;i < 3; i++)
	{
		if(udbp->bp[i].pageNum == pageNum)
		{
			...
			//printf("fix count inside for loop::%d\n",udbp->fixcount[i]);
...
//			ensureCapacity (pageNum, &fHandle);
		...
		for(i = 0;i < 3;i++)
		{
			...
			else if( maxNum >= 2 && RC != RC_BM_PAGE_INSERTED )//
			{
...
RC markDirty (BM_BufferPool *const bm, BM_PageHandle *const page)
{
	...
	for(i =0 ; i< 3; i++)
	{
		...
		if(udbp->bp[i].pageNum == page->pageNum)
		{
			udbp->dirty[i] = true;
			//printf("i:: %d\n",i);
			//printf("pageNum :: %d  --------    udbp->dirty[i]:: %d\n",page->pageNum,udbp->dirty[i]);
...
RC unpinPage (BM_BufferPool *const bm, BM_PageHandle *const page)
{
	...
	for(i =0 ; i< 3; i++)
	{
		...
		if(udbp->bp[i].pageNum == page->pageNum)
		{
			...
			//printf("udbp->bp[i].fixcount :: %d\n",udbp->fixcount[i]);
		
			if(udbp->dirty[i] == true)// && udbp->fixcount[i]==0)
			{
				//printf("udbp->bp[i].pageNum :: %d\n",udbp->bp[i].pageNum);
		
				//printf("inside the unipin fixcount zero condition\n");
